function [obj,beta,t,theta,stq] =qoqc_weighted_LSSVM_Linf_multiclass_regularized(Kmix,L,lambda,lb)
...
% lb is the lowerbound of the kernel coefficients
%
% output variables:
...
for kernelLoop = 1:1:numK
	...
	prob.qoval =  [prob.qoval 1/lambda.*w'];
end
...
theta = res.sol.itr.suc(1:numP).*2;

%theta = theta + lb/numP;


for loop=1:1:size(theta,1)
   theta(loop) = (1-lb)*theta(loop)+lb/size(theta,1);
end
